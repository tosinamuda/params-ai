...
const AddInterface = () => {
  ...
  const { mutate: doAddInterface, isPending: isAddInterfaceLoading } =
    trpc.interface.create.useMutation();
  ...
  const onSubmit: SubmitHandler<InterfaceSchemaType> = (data) => {
    ...
    doAddInterface(
      ...
      {
        onSuccess: (data) => {
          utils.interface.list.setData(undefined, (oldData) =>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white flex flex-col items-center justify-center px-8 py-4 border border-gray-200 shadow-sm rounded-lg">
          <div className="flex flex-col px-4 py-4">
            <h2 className="text-lg font-semibold">Add Interface Type</h2>
            <p className="text-sm text-[#415163]">
              Different interface types are used in experiments for example in
              comparing a chat interface to form interface, we need to add two
              interfaces to allow us use them in our experiments
            </p>
            <form
            ...
            >
              <div className="flex flex-col justify-center">
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      <input
                        ...
                        placeholder={`Enter Interface Type Name`}
                        {...register("name")}
                      />
                      {errors.name && (
                ...
                <div className="flex flex-row justify-between items-center">
                  <button
                    className="py-2 px-5 rounded bg-indigo-500 text-center text-white hover:text-indigo-800 hover:bg-white hover:outline-violet-400 disabled:bg-gray-100 disabled:hover:bg-gray-100 disabled:text-slate-700"
                    type="submit"
                    disabled={isAddInterfaceLoading}
                  >
                    Add Interface
                  </button>
                </div>
        ...
        <div className="px-8 py-4 border border-gray-200 shadow-sm rounded-lg">
          <div className="px-4 py-4  w-full">
            <h2 className="text-lg font-semibold">List of Interface Types</h2>
